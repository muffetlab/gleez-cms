...
<?php else:?>
	...
	<div class="content">
        <?php echo Form::open($action, ['id' => 'admin-blog-form', 'class' => 'no-form']); ?>
				...
				<div class="row">
					<div class="form-group col-xs-7 col-sm-3 col-md-2">
						<div class="control-group <?php echo isset($errors['operation']) ? 'has-error': ''; ?>">
                            <?php echo Form::select('operation', Post::bulk_actions(TRUE, 'blog'), '', ['class' => 'form-control col-md-5']); ?>
						</div>
					</div>
					<div class="form-group col-xs-5 col-sm-2 col-md-2">
                        <?php echo Form::submit('blog-bulk-actions', __('Apply'), ['class' => 'btn btn-default col-md-5']); ?>
					</div>
					<div class="form-group col-xs-6 col-sm-7 col-md-8 form-actions-right">
                        <?php echo HTML::anchor(Route::get('blog')->uri(['action' => 'add']), '<i class="fas fa-plus"></i> ' . __('New entry'), ['class' => 'btn btn-success pull-right']); ?>
					</div>
				</div>
			</fieldset>
